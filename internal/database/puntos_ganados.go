...
func (db *DB) CreatePuntosGanados(ctx context.Context, idRegistroEvaluacion, idTask *int, points int, fechaRegistro time.Time) (int, error) {
	var regEvalParam, taskParam interface{}
	...
	query := `INSERT INTO puntos_ganados (id_registro_evaluacion, id_task, points, fecha_registro)
		VALUES ($1, $2, $3, $4) RETURNING id`

	var id int
	err := db.Pool.QueryRow(ctx, query, regEvalParam, taskParam, points, fechaRegistro).Scan(&id)
	return id, err
}

// GetPuntosGanados obtiene todos los registros
func (db *DB) GetPuntosGanados(ctx context.Context) ([]PuntosGanados, error) {
	query := `SELECT id, id_registro_evaluacion, id_task, points, fecha_registro
	...
	for rows.Next() {
		var p PuntosGanados
		if err := rows.Scan(&p.ID, &p.IDRegistroEvaluacion, &p.IDTask, &p.Points, &p.FechaRegistro); err != nil {
			return nil, err
		}
...
func (db *DB) GetPuntosGanado(ctx context.Context, id int) (PuntosGanados, error) {
	...
	var p PuntosGanados
	err := db.Pool.QueryRow(ctx, query, id).Scan(&p.ID, &p.IDRegistroEvaluacion, &p.IDTask, &p.Points, &p.FechaRegistro)
	if err != nil {
...
func (db *DB) UpdatePuntosGanados(ctx context.Context, id int, idRegistroEvaluacion, idTask *int, points int, fechaRegistro time.Time) (int, error) {
	var regEvalParam, taskParam interface{}
	if idRegistroEvaluacion != nil {
		regEvalParam = *idRegistroEvaluacion
	} else {
	...
	query := `UPDATE puntos_ganados
		SET id_registro_evaluacion = $1, id_task = $2, points = $3, fecha_registro = $4
		WHERE id = $5 RETURNING id`

	var updatedID int
	err := db.Pool.QueryRow(ctx, query, regEvalParam, taskParam, points, fechaRegistro, id).Scan(&updatedID)
